...
class SystemAdminService(ServiceEngineerService):
    ...
    def add_user(self, allowed_roles: list, required_fields: dict) -> bool:
        if required_fields["role"] in allowed_roles and self.session.is_valid() and self.session.role in ["super_admin", "system_admin"]:
            cipher = Fernet(os.getenv("FERNET_KEY").encode())
    ...
    def update_user(self, id: int, to_update: dict):
        ...
        if (role != "super_admin" and self.session.is_valid() and self.session.role in role_list and self.session.role != role):
    ...
    def update_own_account(self):
        ...
        if self.session.is_valid() and self.session.role == "system_admin":
    ...
    def reset_password(self, id, allowed_roles, temp_password):
        ...
        if self.session.is_valid() and self.session.role in ["super_admin", "system_admin"] and auth.get_role_id(id) in allowed_roles:
        ...
        elif not self.session.is_valid() or self.session.role not in ["super_admin", "system_admin"]:
    ...
    def restore_backup_with_code(self, code) -> bool:
        if self.session.is_valid() and self.session.role in ["system_admin", "super_admin"]:
    ...
    def view_restore_codes(self, sys=True):
        if self.session.is_valid() and self.session.role in ["super_admin", "system_admin"]:
            ...
            print("Unauthorized or session expired.")
            log_to_db({"username": self.session.user, "activity": "tried to view restore codes", "additional_info": "not an admin", "suspicious": 1})
            return False
    ...
    def delete_account(self) -> None:
        if self.session.is_valid() and self.session.role == "system_admin":
            ...
            log_to_db({"username": self.session.user, "activity": "Tried to delete their account", "additional_info": "Not a sysadmin", "suspicious": 1})
            print("Session expired.")
            return
    ...
    def get_user_by_id(self, user_id: int) -> dict | None:
        if self.session.is_valid() and self.session.role in ["system_admin", "super_admin"]:
            ...
            log_to_db({"username": self.session.user, "activity": "Tried to get user by id", "additional_info": "Not an admin", "suspicious": 1})
            print("Session expired.")
            return
